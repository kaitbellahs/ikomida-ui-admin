<script lang="ts">
  ...
  $: if (searchTerm !== oldValue) {
    error = false
    if (searchTerm.length > 0 && contract?.id) {
      Stores.Loading.instance.start()
      search(contract.id, searchTerm).then(result => {
  ...
  async function productUp(categoryId?: string, id?: string) {
    ...
    const response = await updateProduct(contract.id, objects)
    if (response?.success) {
  ...
  async function categoryUp(id?: string) {
    ...
    const response = await updateCategory(contract.id, objects)
    if (response?.success) {
...
<Views.Button disabled={categoriesAndProducts.length <= 0} on:click={newProduct}
  ><Fa icon={faEdit} /> <span>Novo produto</span></Views.Button
>
<Views.Button on:click={newCategory}><Fa icon={faEdit} /> <span>Nova categoria</span></Views.Button>
<Views.Divider />
