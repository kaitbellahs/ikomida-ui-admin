<script lang="ts">
  ...
  } from '../../../network/VendorSettings.js'
  ...
  $: if (integratePagSeguro.callback) {
    ...
    if (integratePagSeguro.url) {
      ...
      if (!contract?.id) {
        ...
        updatePaymentGateway(contract.id, payload).then(response => {
          if (!response.success) {
  ...
  async function updateDelivery() {
    ...
    const response = await setDelivery(contract.id, vendorSettings)
    if (!response.success) {
      Stores.MessageAlert.instance.show(response.data)
    }
    Stores.Loading.instance.stop()
  ...
  async function deIntegrationPagSeguro() {
    ...
    const response = await revokePaymentGateway(contract.id)
    if (response.success) {
      paymentGateway = Types.Classes.CVendorPaymentGateway.fromObject(response.data)
  ...
  async function requestPagSeguroIntegration() {
    ...
    if (paymentGateway?.integrated) {
      ...
      const response = await getPagSeguroUrl(contract.id)
      if (response.success) {
  ...
  onMount(async () => {
    ...
    const response = await getSettings(contract.id)
    if (response.success) {
